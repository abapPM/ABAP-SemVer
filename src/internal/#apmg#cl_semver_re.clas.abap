CLASS /apmg/cl_semver_re DEFINITION
  PUBLIC
  CREATE PUBLIC.
...
CLASS /apmg/cl_semver_re IMPLEMENTATION.
  ...
  METHOD class_constructor.
    ...
    create_token(
      name  = 'COERCEPLAIN'
      value = |(^\|[^\\d])(\\d\{1,{ /apmg/if_semver_constants=>max_safe_component_length }\})| &&
              |(?:\\.(\\d\{1,{ /apmg/if_semver_constants=>max_safe_component_length }\}))?| &&
              |(?:\\.(\\d\{1,{ /apmg/if_semver_constants=>max_safe_component_length }\}))?| ).
    create_token(
      name  = 'COERCE'
  ...
  METHOD make_safe_regex.
    ...
    DATA(safe_regex_replacements) = VALUE ty_regex_replacements(
      ( token = `\s`               max = 1 )
      ( token = `\d`               max = /apmg/if_semver_constants=>max_length )
      ( token = letter_dash_number max = /apmg/if_semver_constants=>max_safe_build_length ) ).
