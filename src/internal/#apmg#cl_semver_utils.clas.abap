CLASS /apmg/cl_semver_utils DEFINITION
  PUBLIC
  CREATE PUBLIC.
...
CLASS /apmg/cl_semver_utils IMPLEMENTATION.
  ...
  METHOD version_trim.
    ...
    result = replace(
      val   = trim( data )
      regex = /apmg/cl_semver_re=>token-vtrim-src
      with  = /apmg/cl_semver_re=>version_trim_replace
      occ   = /apmg/cl_semver_re=>token-vtrim-occ ) ##REGEX_POSIX.
  ENDMETHOD.
ENDCLASS.
