CLASS /apmg/cl_semver_range DEFINITION
  PUBLIC
  CREATE PUBLIC.
...
************************************************************************
  PUBLIC SECTION.
    ...
    TYPES:
      ty_comparators TYPE STANDARD TABLE OF REF TO /apmg/cl_semver_comparator WITH KEY table_line,
    ...
    CLASS-METHODS create
      IMPORTING
        ...
        !incpre       TYPE abap_bool DEFAULT abap_false
      RETURNING
        VALUE(result) TYPE REF TO /apmg/cl_semver_range
      RAISING
        /apmg/cx_error.
    ...
    METHODS intersects
      IMPORTING
        !range        TYPE REF TO /apmg/cl_semver_range
        !loose        TYPE abap_bool DEFAULT abap_false
        !incpre       TYPE abap_bool DEFAULT abap_false
      RETURNING
        VALUE(result) TYPE abap_bool
  ...
  PRIVATE SECTION.
    ...
    DATA:
      ...
      options   TYPE /apmg/if_semver_options=>ty_options.

    CLASS-METHODS is_any
      IMPORTING
        !comp         TYPE REF TO /apmg/cl_semver_comparator
      RETURNING
        VALUE(result) TYPE abap_bool.
...
CLASS /apmg/cl_semver_range IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    raw = /apmg/cl_semver_utils=>trim( range ).
    ...
    LOOP AT ranges ASSIGNING FIELD-SYMBOL(<range>).
      INSERT parse_range( /apmg/cl_semver_utils=>trim( <range> ) ) INTO TABLE set.
    ENDLOOP.
    ...
    IF set IS INITIAL.
      RAISE EXCEPTION TYPE /apmg/cx_error_text EXPORTING text = |Invalid SemVer Range: { raw }|.
    ENDIF.
  ...
  METHOD create.
    ...
    DATA comp TYPE REF TO /apmg/cl_semver_comparator.

    DATA(kind) = cl_abap_typedescr=>describe_by_data( range )->type_kind.

    IF kind = cl_abap_typedescr=>typekind_oref AND range IS INSTANCE OF /apmg/cl_semver_range.
      ...
      result = NEW /apmg/cl_semver_range( range = |{ result->raw }| loose = loose incpre = incpre ).

    ELSEIF kind = cl_abap_typedescr=>typekind_oref AND range IS INSTANCE OF /apmg/cl_semver_comparator.

      comp ?= range.

      result = NEW /apmg/cl_semver_range( range = |{ comp->value }| loose = loose incpre = incpre ).

    ELSEIF kind = cl_abap_typedescr=>typekind_char OR kind = cl_abap_typedescr=>typekind_string.

      result = NEW /apmg/cl_semver_range( range = |{ range }| loose = loose incpre = incpre ).

    ELSE.
      RAISE EXCEPTION TYPE /apmg/cx_error_text EXPORTING text = 'Invalid parameter type'.
    ENDIF.
  ...
  METHOD is_satisfiable.
    ...
    WHILE result = abap_true AND lines( remaining_comparators ) > 0.
      ...
      LOOP AT remaining_comparators ASSIGNING FIELD-SYMBOL(<other_comparator>).
        DATA(semcomp) = /apmg/cl_semver_comparator=>create( test_comparator ).
  ...
  METHOD parse_range.
    ...
    range = replace(
      val   = range
      regex = /apmg/cl_semver_re=>token-comparatortrim-safe_src
      with  = /apmg/cl_semver_re=>comparator_trim_replace
      occ   = /apmg/cl_semver_re=>token-comparatortrim-occ ) ##REGEX_POSIX.

    " `~ 1.2.3` => `~1.2.3`
    range = replace(
      val   = range
      regex = /apmg/cl_semver_re=>token-tildetrim-safe_src
      with  = /apmg/cl_semver_re=>tilde_trim_replace
      occ   = /apmg/cl_semver_re=>token-tildetrim-occ ) ##REGEX_POSIX.

    " `^ 1.2.3` => `^1.2.3`
    range = replace(
      val   = range
      regex = /apmg/cl_semver_re=>token-carettrim-safe_src
      with  = /apmg/cl_semver_re=>caret_trim_replace
      occ   = /apmg/cl_semver_re=>token-carettrim-occ ) ##REGEX_POSIX.
    ...
    IF options-loose = abap_true.
      ...
      DATA(r) = /apmg/cl_semver_re=>token-comparatorloose-safe_regex.
      LOOP AT comps ASSIGNING <comp>.
    ...
    LOOP AT comps ASSIGNING <comp>.
      DATA(semcomp) = /apmg/cl_semver_comparator=>create(
        comp   = <comp>
        loose  = options-loose
    ...
    IF formatted IS INITIAL.
      LOOP AT set ASSIGNING FIELD-SYMBOL(<set>).
        LOOP AT <set> ASSIGNING FIELD-SYMBOL(<comp>).
          IF sy-tabix = 1.
            DATA(comp) = /apmg/cl_semver_utils=>trim( <comp>->value ).
          ELSE.
            comp = comp && ` ` && /apmg/cl_semver_utils=>trim( <comp>->value ).
          ENDIF.
        ENDLOOP.
        INSERT comp INTO TABLE comps.
      ENDLOOP.

      formatted = /apmg/cl_semver_utils=>trim( concat_lines_of( table = comps sep = `||` ) ).
    ENDIF.
  ...
  METHOD replace_caret.
    ...
    DATA(r) = COND #(
      ...
      THEN /apmg/cl_semver_re=>token-caretloose-safe_regex
      ELSE /apmg/cl_semver_re=>token-caret-safe_regex ).
    ...
    TRY.
      ...
      CATCH cx_sy_arithmetic_overflow.
        RAISE EXCEPTION TYPE /apmg/cx_error_text EXPORTING text = 'Overflow'.
  ...
  METHOD replace_carets.
    ...
    SPLIT /apmg/cl_semver_utils=>trim( comp ) AT ` ` INTO TABLE DATA(comps).
  ...
  METHOD replace_gte0.
    ...
    DATA(regex) = COND #(
      ...
      THEN /apmg/cl_semver_re=>token-gte0pre-safe_src
      ELSE /apmg/cl_semver_re=>token-gte0-safe_src ).

    result = replace(
      val   = /apmg/cl_semver_utils=>trim( comp )
      regex = regex
      with  = '' ) ##REGEX_POSIX.
  ...
  METHOD replace_hyphen.
    ...
    DATA(r) = COND #(
      WHEN loose = abap_true
      THEN /apmg/cl_semver_re=>token-hyphenrangeloose-safe_regex
      ELSE /apmg/cl_semver_re=>token-hyphenrange-safe_regex ).
    ...
    TRY.
        ...
        IF m->match( ).
          ...
          DATA(with) = /apmg/cl_semver_utils=>trim( |{ from } { to }| ).
  ...
  METHOD replace_stars.
    ...
    result = replace(
      val   = /apmg/cl_semver_utils=>trim( comp )
      regex = /apmg/cl_semver_re=>token-star-safe_src
      with  = '' ) ##REGEX_POSIX.
  ...
  METHOD replace_tilde.
    ...
    DATA(r) = COND #(
      WHEN loose = abap_true
      THEN /apmg/cl_semver_re=>token-tildeloose-safe_regex
      ELSE /apmg/cl_semver_re=>token-tilde-safe_regex ).
  ...
  METHOD replace_xrange.
    ...
    result = /apmg/cl_semver_utils=>trim( comp ).

    DATA(r) = COND #(
      WHEN loose = abap_true
      THEN /apmg/cl_semver_re=>token-xrangeloose-safe_regex
      ELSE /apmg/cl_semver_re=>token-xrange-safe_regex ).
  ...
  METHOD test.
    ...
    TRY.
        DATA(semver) = /apmg/cl_semver=>create(
          version = version
          loose   = options-loose
          incpre  = options-incpre ).
      CATCH /apmg/cx_error.
  ...
  METHOD test_set.
    ...
    DATA(semver) = /apmg/cl_semver=>create( version = version loose = loose incpre = incpre ).
    ...
    IF semver->prerelease IS NOT INITIAL AND incpre = abap_false.
      ...
      LOOP AT comparators ASSIGNING <comparator>.
        IF <comparator>->semver = /apmg/cl_semver_comparator=>any_semver.
          CONTINUE.
        ENDIF.
