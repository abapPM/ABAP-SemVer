...
CLASS zcl_semver_range IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    IF set IS INITIAL.
      RAISE EXCEPTION TYPE zcx_error_text EXPORTING text = |Invalid SemVer Range: { raw }|.
    ENDIF.
  ...
  METHOD parse_range.
    ...
    LOOP AT comps ASSIGNING <comp>.
      DATA(semcomp) = zcl_semver_comparator=>create(
        comp   = <comp>
        loose  = options-loose
    ...
    IF formatted IS INITIAL.
      LOOP AT set ASSIGNING FIELD-SYMBOL(<set>).
        LOOP AT <set> ASSIGNING FIELD-SYMBOL(<comp>).
          IF sy-tabix = 1.
            DATA(comp) = zcl_semver_utils=>trim( <comp>->value ).
          ELSE.
  ...
  METHOD replace_caret.
    ...
    TRY.
      ...
      CATCH cx_sy_arithmetic_overflow.
        RAISE EXCEPTION TYPE zcx_error_text EXPORTING text = 'Overflow'.
