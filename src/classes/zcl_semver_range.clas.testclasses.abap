...
CLASS zcl_semver_range DEFINITION LOCAL FRIENDS ltcl_semver_range.
...
CLASS ltcl_semver_range IMPLEMENTATION.
  ...
  METHOD range_parse.
    ...
    LOOP AT zcl_semver_fixtures=>range_parse( ) INTO DATA(range_parse).
      ...
      ELSE.
        ...
        DATA(e) = zcl_semver_range=>create(
 ...
 " formatted value is calculated lazily and cached
  ...
  METHOD create_from_range.
    ...
    DATA(loose) = zcl_semver_range=>create(
      range  = '1.2.3'
      loose  = abap_true ).
    ...
    DATA(incpre) = zcl_semver_range=>create(
      range  = '1.2.3'
      incpre = abap_true ).
