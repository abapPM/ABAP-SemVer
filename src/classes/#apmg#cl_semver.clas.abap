CLASS /apmg/cl_semver DEFINITION
  PUBLIC
  CREATE PRIVATE.
...
CLASS /apmg/cl_semver IMPLEMENTATION.
  ...
  METHOD compare_build.
    ...
    DO.
      ...
      ELSEIF a IS INITIAL.
        ...
        RETURN.
      ELSEIF a <> b.
        result = /apmg/cl_semver_identifiers=>compare_identifiers( a = a b = b ).
        RETURN.
      ENDIF.
      i = i + 1.
    ENDDO.
  ...
  METHOD constructor.
    ...
    IF strlen( version ) > /apmg/if_semver_constants=>max_length.
      RAISE EXCEPTION TYPE /apmg/cx_error_text
        EXPORTING
          text = |Version is longer than { /apmg/if_semver_constants=>max_length } characters|.
    ENDIF.
    ...
    DATA(r) = COND #(
      ...
      THEN /apmg/cl_semver_re=>token-loose-safe_regex
      ELSE /apmg/cl_semver_re=>token-full-safe_regex ).

    TRY.
        DATA(m) = r->create_matcher( text = /apmg/cl_semver_utils=>version_trim( version ) ).
        ...
        IF major_num BETWEEN 0 AND /apmg/if_semver_constants=>max_safe_integer.
          major = major_num.
        ELSE.
          RAISE EXCEPTION TYPE /apmg/cx_error_text
            EXPORTING
        ...
        IF minor_num BETWEEN 0 AND /apmg/if_semver_constants=>max_safe_integer.
          minor = minor_num.
        ELSE.
          RAISE EXCEPTION TYPE /apmg/cx_error_text
            EXPORTING
        ...
        IF patch_num BETWEEN 0 AND /apmg/if_semver_constants=>max_safe_integer.
          patch = patch_num.
        ELSE.
          RAISE EXCEPTION TYPE /apmg/cx_error_text
            EXPORTING
        ...
        IF m4 IS NOT INITIAL.
          ...
          LOOP AT prerelease ASSIGNING FIELD-SYMBOL(<pre>).
            IF /apmg/cl_semver_utils=>is_numeric( <pre> ).
  ...
  METHOD create.
    ...
    IF kind = cl_abap_typedescr=>typekind_oref AND version IS INSTANCE OF /apmg/cl_semver.
      ...
      result = NEW /apmg/cl_semver( version = |{ result->version }| loose = loose incpre = incpre ).

    ELSEIF kind = cl_abap_typedescr=>typekind_char OR kind = cl_abap_typedescr=>typekind_string.

      result = NEW /apmg/cl_semver( version = |{ version }| loose = loose incpre = incpre ).
    ...
    CASE release_type.
      ...
      WHEN 'pre'.
        ...
        IF identifier_base IS INITIAL OR identifier_base = `0`.
          DATA(base) = `0`.
        ELSE.
          base = COND #( WHEN /apmg/cl_semver_utils=>is_numeric( identifier_base ) THEN `1` ELSE `0` ).
        ENDIF.
        ...
        ELSE.
          ...
          WHILE i > 0.
            IF /apmg/cl_semver_utils=>is_numeric( prerelease[ i ] ).
