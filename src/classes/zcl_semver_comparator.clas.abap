...
************************************************************************
  PUBLIC SECTION.
    ...
    CLASS-DATA any_semver TYPE REF TO zcl_semver READ-ONLY.
...
CLASS zcl_semver_comparator IMPLEMENTATION.
  ...
  METHOD create.
    ...
    ELSE.
      RAISE EXCEPTION TYPE zcx_error_text EXPORTING text = 'Invalid parameter type'.
    ENDIF.
  ...
  METHOD intersects.
    ...
    IF comp IS INITIAL.
      RAISE EXCEPTION TYPE zcx_error_text EXPORTING text = 'A comparator is required'.
    ENDIF.
...
*      opposite_directions_greater = abap_true )
  ...
  METHOD parse.
    ...
    TRY.
        ...
        IF NOT m->match( ).
          RAISE EXCEPTION TYPE zcx_error_text EXPORTING text = |Invalid comparator: { comp }|.
        ENDIF.
      ...
      CATCH cx_sy_matcher.
        RAISE EXCEPTION TYPE zcx_error_text EXPORTING text = |Error evaluating regex for { comp }|.
    ENDTRY.
