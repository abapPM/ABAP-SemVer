...
CLASS zcl_semver_comparator IMPLEMENTATION.
  ...
  METHOD create.
    ...
    ELSE.
      zcx_error=>raise( 'Invalid parameter type' ).
    ENDIF.
  ...
  METHOD intersects.
    ...
    IF comp IS INITIAL.
      zcx_error=>raise( 'A comparator is required' ).
    ENDIF.
...
*      opposite_directions_greater = abap_true )
  ...
  METHOD parse.
    ...
    TRY.
        ...
        IF NOT m->match( ).
          zcx_error=>raise( |Invalid comparator: { comp }| ).
        ENDIF.
      ...
      CATCH cx_sy_matcher.
        zcx_error=>raise( |Error evaluating regex for { comp }| ).
    ENDTRY.
