...
CLASS zcl_semver IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    IF strlen( version ) > zif_semver_constants=>max_length.
      zcx_error=>raise( |Version is longer than { zif_semver_constants=>max_length } characters| ).
    ENDIF.
    ...
    TRY.
        ...
        IF NOT m->match( ).
          zcx_error=>raise( |Invalid version: { version }| ).
        ENDIF.
        ...
        IF major_num BETWEEN 0 AND zif_semver_constants=>max_safe_integer.
          major = major_num.
        ELSE.
          zcx_error=>raise( |Invalid major version: { major_num }| ).
        ENDIF.

        IF minor_num BETWEEN 0 AND zif_semver_constants=>max_safe_integer.
          minor = minor_num.
        ELSE.
          zcx_error=>raise( |Invalid minor version: { minor_num }| ).
        ENDIF.

        IF patch_num BETWEEN 0 AND zif_semver_constants=>max_safe_integer.
          patch = patch_num.
        ELSE.
          zcx_error=>raise( |Invalid patch version: { patch_num }| ).
        ENDIF.
      ...
      CATCH cx_sy_matcher.
        zcx_error=>raise( |Error evaluating regex for { version }| ).
    ENDTRY.
  ...
  METHOD create.
    ...
    ELSE.
      zcx_error=>raise( |Invalid version. Must be a string or a semver. Got { descr->absolute_name }| ).
    ENDIF.
        ...
        ELSE.
          ...
          IF i = 0.
            ...
            IF identifier = prerelease_string AND identifier_base = false.
              zcx_error=>raise( 'Invalid increment argument: identifier already exists' ).
            ENDIF.
