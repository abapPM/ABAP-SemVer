CLASS ltcl_semver_ranges DEFINITION FOR TESTING RISK LEVEL HARMLESS
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      ...
      max_satisfying FOR TESTING RAISING /apmg/cx_error,
      min_satisfying FOR TESTING RAISING /apmg/cx_error,
      min_version FOR TESTING RAISING /apmg/cx_error,
      outside FOR TESTING RAISING /apmg/cx_error,
      simplify FOR TESTING RAISING /apmg/cx_error,
      subset FOR TESTING RAISING /apmg/cx_error,
      to_comparators FOR TESTING RAISING /apmg/cx_error,
      valid_range FOR TESTING RAISING /apmg/cx_error.
...
CLASS ltcl_semver_ranges IMPLEMENTATION.
  ...
  METHOD intersects.
    ...
    LOOP AT /apmg/cl_semver_fixtures=>comparator_intersection( ) INTO DATA(comparator_intersection).
      ...
      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>intersects( r1 = comp0 r2 = comp1 )
        exp = comparator_intersection-res
        msg = msg ).
      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>intersects( r1 = comp1 r2 = comp0 )
        exp = comparator_intersection-res
        msg = msg ).

      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>intersects( r1 = comp0 r2 = comp1 loose = abap_true )
        exp = comparator_intersection-res
        msg = msg ).
      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>intersects( r1 = comp1 r2 = comp0 loose = abap_true )
        exp = comparator_intersection-res
        msg = msg ).

      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>intersects( r1 = comparator_intersection-c0 r2 = comparator_intersection-c1 )
        exp = comparator_intersection-res
        msg = msg ).
      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>intersects( r1 = comparator_intersection-c1 r2 = comparator_intersection-c0 )
        exp = comparator_intersection-res
        msg = msg ).

      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>intersects( r1 = comparator_intersection-c0 r2 = comparator_intersection-c1 loose = abap_true )
        exp = comparator_intersection-res
        msg = msg ).
      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>intersects( r1 = comparator_intersection-c1 r2 = comparator_intersection-c0 loose = abap_true )
        exp = comparator_intersection-res
        msg = msg ).
    ENDLOOP.
    ...
    LOOP AT /apmg/cl_semver_fixtures=>range_intersection( ) INTO DATA(range_intersection).
      ...
      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>intersects( r1 = range0 r2 = range1 )
        exp = range_intersection-res
        msg = msg ).
      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>intersects( r1 = range1 r2 = range0 )
        exp = range_intersection-res
        msg = msg ).

      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>intersects( r1 = range0 r2 = range1 loose = abap_true )
        exp = range_intersection-res
        msg = msg ).
      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>intersects( r1 = range1 r2 = range0 loose = abap_true )
        exp = range_intersection-res
        msg = msg ).

      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>intersects( r1 = range_intersection-r0 r2 = range_intersection-r1 )
        exp = range_intersection-res
        msg = msg ).
      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>intersects( r1 = range_intersection-r1 r2 = range_intersection-r0 )
        exp = range_intersection-res
        msg = msg ).

      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>intersects( r1 = range_intersection-r0 r2 = range_intersection-r1 loose = abap_true )
        exp = range_intersection-res
        msg = msg ).
      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>intersects( r1 = range_intersection-r1 r2 = range_intersection-r0 loose = abap_true )
        exp = range_intersection-res
        msg = msg ).
    ENDLOOP.
  ...
  METHOD max_satisfying.
    ...
    LOOP AT tests INTO DATA(test).
      ...
      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>max_satisfying( versions = versions range = test-range loose = test-loose )
        exp = test-res
    ...
    cl_abap_unit_assert=>assert_equals(
      act = /apmg/cl_semver_ranges=>max_satisfying( versions = versions range = range )
      exp = ''
      msg = |{ range }| ).
  ...
  METHOD min_satisfying.
    ...
    LOOP AT tests INTO DATA(test).
      ...
      cl_abap_unit_assert=>assert_equals(
        act = /apmg/cl_semver_ranges=>min_satisfying( versions = versions range = test-range loose = test-loose )
        exp = test-res
        msg = |{ test-versions } { test-range } { test-res }| ).
    ENDLOOP.
    ...
    cl_abap_unit_assert=>assert_equals(
      act = /apmg/cl_semver_ranges=>min_satisfying( versions = versions range = range )
      exp = ''
      msg = |{ range }| ).
  ...
  METHOD outside.
    ...
    LOOP AT /apmg/cl_semver_fixtures=>version_gt_range( ) INTO DATA(version_gt_range).
      DATA(msg) = |{ version_gt_range-range } { version_gt_range-version } { version_gt_range-loose }|.
      DATA(act) = /apmg/cl_semver_ranges=>outside(
        version = version_gt_range-version
        range   = version_gt_range-range
        hilo    = '>'
        loose   = version_gt_range-loose ).
  ...
  METHOD simplify.
    ...
    cl_abap_unit_assert=>assert_equals(
      act = /apmg/cl_semver_ranges=>simplify(
              versions = versions
              range    = '1.x' )
      exp = '1.x' ).
    ...
    DATA(range) = /apmg/cl_semver_range=>create( '1.0.0 || 1.0.1 || 1.0.2 || 1.0.3 || 1.0.4' ).
  ...
  METHOD to_comparators.
    ...
    LOOP AT tests INTO DATA(test).
      DATA(comparators) = /apmg/cl_semver_ranges=>to_comparators( test-range ).
