...
CLASS zcl_semver_ranges IMPLEMENTATION.
    ...
    LOOP AT versions ASSIGNING FIELD-SYMBOL(<version>).
      IF semrange->test( <version> ).
        ...
        IF max IS INITIAL OR maxsv->compare( <version> ) = -1.
          " compare(max, v, true)
          max = <version>.
          maxsv = zcl_semver=>create( version = max loose = loose incpre = incpre ).
        ENDIF.
      ENDIF.
    ENDLOOP.
    ...
    LOOP AT versions ASSIGNING FIELD-SYMBOL(<version>).
      IF semrange->test( <version> ).
        ...
        IF min IS INITIAL OR minsv->compare( <version> ) = +1.
          " compare(min, v, true)
          min = <version>.
          minsv = zcl_semver=>create( version = min loose = loose incpre = incpre ).
        ENDIF.
      ENDIF.
    ENDLOOP.
  ...
  METHOD min_version.
    ...
    DATA setmin TYPE REF TO zcl_semver.

    DATA(semrange) = zcl_semver_range=>create( range = range loose = loose incpre = incpre ).

    DATA(minver) = zcl_semver=>create( '0.0.0' ).
    IF semrange->test( minver ).
      result = minver.
      RETURN.
    ENDIF.

    minver = zcl_semver=>create( '0.0.0-0' ).
    IF semrange->test( minver ).
      result = minver.
      RETURN.
    ENDIF.
    ...
    LOOP AT semrange->set ASSIGNING FIELD-SYMBOL(<set>).
      ...
      LOOP AT comparators ASSIGNING FIELD-SYMBOL(<comparator>).
        ...
        DATA(compver) = zcl_semver=>create( <comparator>->semver->version ).
        ...
        CASE <comparator>->operator.
          ...
          WHEN OTHERS.
            RAISE EXCEPTION TYPE zcx_error_text EXPORTING text = |Unexpected operation: { <comparator>->operator }|.
        ENDCASE.
      ENDLOOP.
  ...
  METHOD outside.
    ...
    IF hilo NA '<>'.
      RAISE EXCEPTION TYPE zcx_error_text EXPORTING text = 'Must provide a hilo val of "<" or ">"'.
    ENDIF.
        ...
        CASE hilo.
          WHEN '>'.
            IF zcl_semver_functions=>gt(
              ...
              high = <comparator>.
            ELSEIF zcl_semver_functions=>lt(
              a      = <comparator>->semver
              b      = low->semver
              loose  = loose
              incpre = incpre ).
          ...
          WHEN '<'.
            IF zcl_semver_functions=>lt(
              ...
              high = <comparator>.
            ELSEIF zcl_semver_functions=>gt(
              a      = <comparator>->semver
              b      = low->semver
              loose  = loose
              incpre = incpre ).
  ...
  METHOD simplify.
    ...
    IF versions IS INITIAL.
      RAISE EXCEPTION TYPE zcx_error_text EXPORTING text = 'Empty version list'.
    ENDIF.
  ...
  METHOD subset.
    ...
    RAISE EXCEPTION TYPE zcx_error_text EXPORTING text = 'TODO'.
