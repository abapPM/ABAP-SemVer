...
CLASS zcl_semver_ranges IMPLEMENTATION.
  ...
  METHOD gtr.
    result = outside( version = version range = range hilo = '>' loose = loose incpre = incpre ).
  ENDMETHOD.
    ...
    LOOP AT semrange->set ASSIGNING FIELD-SYMBOL(<set>).
      ...
      LOOP AT comparators ASSIGNING FIELD-SYMBOL(<comparator>).
        ...
        CASE <comparator>->operator.
          ...
          WHEN OTHERS.
            zcx_semver_error=>raise( |Unexpected operation: { <comparator>->operator }| ).
        ENDCASE.
      ENDLOOP.
  ...
  METHOD simplify.
    ...
    TYPES:
      BEGIN OF ty_min_max,
        ...
        max TYPE string,
      END OF ty_min_max,
      ty_set TYPE STANDARD TABLE OF ty_min_max WITH DEFAULT KEY.
    ...
    LOOP AT set ASSIGNING FIELD-SYMBOL(<set>).
      ...
      ELSEIF <set>-max IS INITIAL AND <set>-min = v[ 1 ].
