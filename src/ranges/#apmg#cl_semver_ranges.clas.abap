CLASS /apmg/cl_semver_ranges DEFINITION
  PUBLIC
  CREATE PUBLIC.
...
CLASS /apmg/cl_semver_ranges IMPLEMENTATION.
  ...
  METHOD intersects.
    ...
    DATA(semrange1) = /apmg/cl_semver_range=>create( range = r1 loose = loose incpre = incpre ).

    DATA(semrange2) = /apmg/cl_semver_range=>create( range = r2 loose = loose incpre = incpre ).
  ...
  METHOD max_satisfying.
    ...
    DATA:
      max   TYPE string,
      maxsv TYPE REF TO /apmg/cl_semver.

    TRY.
        DATA(semrange) = /apmg/cl_semver_range=>create( range = range loose = loose incpre = incpre ).
      CATCH /apmg/cx_error.
        result = ''.
        RETURN.
    ENDTRY.
    ...
    LOOP AT versions ASSIGNING FIELD-SYMBOL(<version>).
      IF semrange->test( <version> ).
        ...
        IF max IS INITIAL OR maxsv->compare( <version> ) = -1.
          " compare(max, v, true)
          max = <version>.
          maxsv = /apmg/cl_semver=>create( version = max loose = loose incpre = incpre ).
        ENDIF.
      ENDIF.
    ENDLOOP.
  ...
  METHOD min_satisfying.
    ...
    DATA:
      min   TYPE string,
      minsv TYPE REF TO /apmg/cl_semver.
    ...
    LOOP AT versions ASSIGNING FIELD-SYMBOL(<version>).
      IF semrange->test( <version> ).
        ...
        IF min IS INITIAL OR minsv->compare( <version> ) = +1.
          " compare(min, v, true)
          min = <version>.
          minsv = /apmg/cl_semver=>create( version = min loose = loose incpre = incpre ).
        ENDIF.
      ENDIF.
    ENDLOOP.
  ...
  METHOD min_version.
    ...
    DATA setmin TYPE REF TO /apmg/cl_semver.

    DATA(semrange) = /apmg/cl_semver_range=>create( range = range loose = loose incpre = incpre ).

    DATA(minver) = /apmg/cl_semver=>create( '0.0.0' ).
    IF semrange->test( minver ).
      result = minver.
      RETURN.
    ENDIF.

    minver = /apmg/cl_semver=>create( '0.0.0-0' ).
    IF semrange->test( minver ).
      result = minver.
      RETURN.
    ENDIF.
    ...
    LOOP AT semrange->set ASSIGNING FIELD-SYMBOL(<set>).
      ...
      LOOP AT comparators ASSIGNING FIELD-SYMBOL(<comparator>).
        ...
        DATA(compver) = /apmg/cl_semver=>create( <comparator>->semver->version ).
        ...
        CASE <comparator>->operator.
          WHEN '>'.
            ...
            IF setmin IS INITIAL OR /apmg/cl_semver_functions=>gt( a = compver b = setmin ).
              setmin = compver.
            ENDIF.
          ...
          WHEN OTHERS.
            RAISE EXCEPTION TYPE /apmg/cx_error_text EXPORTING text = |Unexpected operation: { <comparator>->operator }|.
        ENDCASE.
      ENDLOOP.

      IF setmin IS NOT INITIAL AND ( minver IS INITIAL OR /apmg/cl_semver_functions=>gt( a = minver b = setmin ) ).
        minver = setmin.
      ENDIF.
    ENDLOOP.
  ...
  METHOD outside.
    ...
    DATA:
      high TYPE REF TO /apmg/cl_semver_comparator,
      low  TYPE REF TO /apmg/cl_semver_comparator.
    ...
    IF hilo NA '<>'.
      RAISE EXCEPTION TYPE /apmg/cx_error_text EXPORTING text = 'Must provide a hilo val of "<" or ">"'.
    ENDIF.
    ...
    IF /apmg/cl_semver_functions=>satisfies(
      version = semver
      range   = semrange
      loose   = loose
      incpre  = incpre ).
    ...
    LOOP AT semrange->set ASSIGNING FIELD-SYMBOL(<set>).
      ...
      LOOP AT comparators ASSIGNING FIELD-SYMBOL(<comparator>).
        IF <comparator> = /apmg/cl_semver_comparator=>any_semver.
          <comparator> = /apmg/cl_semver_comparator=>create( '>=0.0.0' ).
        ENDIF.
        ...
        CASE hilo.
          WHEN '>'.
            IF /apmg/cl_semver_functions=>gt(
              a      = <comparator>->semver
              b      = high->semver
              loose  = loose
              incpre = incpre ).
              ...
              high = <comparator>.
            ELSEIF /apmg/cl_semver_functions=>lt(
              a      = <comparator>->semver
              b      = low->semver
              loose  = loose
              incpre = incpre ).
            ...
            ENDIF.
          WHEN '<'.
            IF /apmg/cl_semver_functions=>lt(
              a      = <comparator>->semver
              b      = high->semver
              loose  = loose
              incpre = incpre ).
              ...
              high = <comparator>.
            ELSEIF /apmg/cl_semver_functions=>gt(
              a      = <comparator>->semver
              b      = low->semver
              loose  = loose
              incpre = incpre ).
      ...
      CASE hilo.
        WHEN '>'.
          IF ( low->operator IS INITIAL OR low->operator = comp ) AND
            /apmg/cl_semver_functions=>lte( a = semver b = low->semver ).

            result = abap_false.
            RETURN.
          ELSEIF low->operator = ecomp AND /apmg/cl_semver_functions=>lt( a = semver b = low->semver ).
            result = abap_false.
            RETURN.
          ENDIF.
        WHEN '<'.
          IF ( low->operator IS INITIAL OR low->operator = comp ) AND
            /apmg/cl_semver_functions=>gte( a = semver b = low->semver ).

            result = abap_false.
            RETURN.
          ELSEIF low->operator = ecomp AND /apmg/cl_semver_functions=>gt( a = semver b = low->semver ).
            result = abap_false.
            RETURN.
          ENDIF.
      ENDCASE.
  ...
  METHOD simplify.
    ...
    IF versions IS INITIAL.
      RAISE EXCEPTION TYPE /apmg/cx_error_text EXPORTING text = 'Empty version list'.
    ENDIF.
    ...
    DATA(v) = /apmg/cl_semver_functions=>sort( list = versions loose = loose incpre = incpre ).
  ...
  METHOD subset.
    ...
    RAISE EXCEPTION TYPE /apmg/cx_error_text EXPORTING text = 'TODO'.
