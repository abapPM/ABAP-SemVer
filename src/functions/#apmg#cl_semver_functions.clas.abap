CLASS /apmg/cl_semver_functions DEFINITION
  PUBLIC
  CREATE PUBLIC.
...
CLASS /apmg/cl_semver_functions IMPLEMENTATION.


  METHOD clean.

    DATA(vers) = replace(
      val   = /apmg/cl_semver_utils=>version_trim( version )
      regex = '^[=v]+'
      with  = '' ).
    ...
    CASE op.
      ...
      WHEN '<='.
        result = lte(
          ...
          incpre = incpre ).
      WHEN OTHERS.
        RAISE EXCEPTION TYPE /apmg/cx_error_text EXPORTING text = |Invalid operator: { op }|.
    ENDCASE.
  ...
  METHOD coerce.
    ...
    IF rtl = abap_false.
      DATA(r) = COND #(
        ...
        THEN /apmg/cl_semver_re=>token-coercefull-safe_src
        ELSE /apmg/cl_semver_re=>token-coerce-safe_src ).
    ...
    ELSE.
      ...
      r = COND #(
        WHEN incpre = abap_true
        THEN /apmg/cl_semver_re=>token-coercertlfull-safe_src
        ELSE /apmg/cl_semver_re=>token-coercertl-safe_src ).
  ...
  METHOD compare.
    ...
    DATA(semver_a) = /apmg/cl_semver=>create( version = a loose = loose incpre = incpre ).
    DATA(semver_b) = /apmg/cl_semver=>create( version = b loose = loose incpre = incpre ).
  ...
  METHOD equality.
    ...
    DATA semver_a TYPE REF TO /apmg/cl_semver.
    DATA semver_b TYPE REF TO /apmg/cl_semver.

    IF a IS BOUND AND a IS INSTANCE OF /apmg/cl_semver AND b IS BOUND AND b IS INSTANCE OF /apmg/cl_semver.
      semver_a ?= a.
      semver_b ?= b.
  ...
  METHOD inc.
    ...
    DATA semver TYPE REF TO /apmg/cl_semver.
    ...
    TRY.
        ...
        IF kind = cl_abap_typedescr=>typekind_oref AND version IS INSTANCE OF /apmg/cl_semver.
          semver ?= version.
          result = /apmg/cl_semver=>create( version = semver->version loose = loose incpre = incpre ).
        ELSE.
  ...
  METHOD major.
    ...
    DATA(semver) = /apmg/cl_semver=>create( version = version loose = loose ).
